...
/// The `SnowConfig` struct allows you to tailor the `SnowView` to your needs with a lot of configuration options.
///
...
///
/// - Parameters:
///     - content: This is an array of `Content` enum objects. You can see its documentation for how to use/implement those  (see ``Content`` for more information). It offers the following cases: `.image`, `.emoji`,  and `.shape`. Default value here is an image of a snow flake you can see in the `Resources` folder..
...
///     - `initialVelocity`: Specifies the initial speed with which snow particles will be emitted from the source. The higher the value the larger the radius of the effects as elements will spread in a larger radius around the source. The type is ``InitialVelocity`` and possible values are `.slow`, `.medium`, and `.fast`. Default value is `.medium`.
